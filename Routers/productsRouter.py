...
def post_product():
    try:
        ...
        for key in product_keys:
            if key not in body:
                return jsonify({"success": False, "results": [], "message": f'{key} missing from body'}), 500
        if 'id' in body:
            del body['id']
...
def patch_product(id):
    try:
        ...
        return jsonify({"success": False, "results": {}, "message": "Product Not Found"}), 200
    except Exception as ex:
        return jsonify({"success": False, "results": [], "message": ex}), 500
...
def get_product_byId(id):
    try:
        ...
        if result:
            ...
            return jsonify({"success": True, "result": result, "message": "OK"}), 200
        return jsonify({"success": False, "results": {}, "message": "Product Not Found"}), 200
    except Exception as ex:
        return jsonify({"success": False, "results": [], "message": ex}), 500
